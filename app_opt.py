...
df2 = pd.read_csv('cost_of_living.csv')
...
def load_model():
    with open('salary_model_rf.pkl', 'rb') as file1:
...
def show_predict_page():
    ...
    developer_type = ('Front-end Developer', 'Back-end Developer',
       'Full-stack Developer', 'Other', 'Data Scientist or ML Specialist',
       'DevOps Specialist', 'Data or Business Analyst',
       'Mobile Developer', 'Academic Reseacher', 'System Administrator',
       'Game Developer', 'QA tester', 'Data Engineer', 'Product Manager',
       'Senior Executive', 'Engineering Manager',
    ...
    if ok:
       ...
       'DevType_Back-end Developer', 'DevType_Data Engineer',
       ...
       'EdLevel_Less than a Bachelor', 'EdLevel_Master’s degree',
       'EdLevel_PhD', 'age_transformed', 'aver_count_salary'])

        
        to_predict.loc[0,f'DevType_{dev_type}'] = 1 
        to_predict.loc[0,f'EdLevel_{education}'] = 1
        if to_predict['EdLevel_PhD'][0]==1:
       ...
       'EdLevel_PhD', 'age_transformed', 'aver_count_salary']]
        ...
        st.subheader(f"The average estimate gross salary is ${salary[0]:.2f}")
...
def show_explore_page():
    ...
    st.write(""" ### Cost of Living Index by Country for 2022""")
    #data2 = df2.groupby(["Country"])["Cost of Livning Index"]
    st.bar_chart(data =df2, x= 'Country', y = 'Cost of Livning Index', height=600.9, width=60.5)
